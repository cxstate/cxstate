type UpdatePartialDef<ContextType, PayloadType> = {
    [P in keyof ContextType]?: (ctx: ContextType, pl: PayloadType) => ContextType[P];
}

export interface EventDef<ContextType, PayloadType=any, NextPayloadType=any> {
  /**
   ...
   * The context must not be changed, otherwise unpredictable behavior might occur.
   */
  cond?: (ctx: ContextType, pl: PayloadType) => boolean
  /**
   ...
   */
  replace?: (ctx: ContextType, pl: PayloadType) => ContextType
  /**
   ...
   */
  update?: UpdatePartialDef<ContextType, PayloadType>
  /**
   ...
   */
  tap?: (ctx: ContextType, pl: PayloadType) => void
  /**
   ...
   * Use the helper fn Next<ContextType, PayloadType, NextPayloadType>(...) then for strong typing.
   */
  next?: string|((ctx: ContextType, inputPl: PayloadType) => [string, NextPayloadType])
}
...
export type SendFn = <PayloadType=any>(name: string, payload?: PayloadType|Promise<PayloadType>) => void;
